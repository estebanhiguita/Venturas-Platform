...
while($row = mysqli_fetch_array($resultrta)){
    ...
    if(in_array("2", $pregunta, TRUE)){
        echo  "<script> alert('Ya has ingresado esta pregunta'); window.location.href='../pregunta1.php#fase2';</script> ";
        //header('Location:../pregunta1.php'); 
        return;
        }else
        {
            $respuesta=$_POST["rta2"];
            $select= "INSERT INTO repuestas(descripcion, usuario_id, preg_id) values ('".$respuesta."','".$usuario."',2)  ;";
            if ($result = mysqli_query($mysqli, $select) ){
